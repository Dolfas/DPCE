...
for k = 1:N - 1 
    % Solve MPC problem
    z = mpc_solve(x0,z0, H, R, A, B, C, ref);
    du(:,k) = z(190);
...
function z = mpc_solve(x0,z0, H, R, A, B, C, ref)
    ...
    % lb(1:188,1) = -inf; ub(1:188,1)= inf;
    % lb(189:209,1)= 0; ub(189:209,1) = 100;  

    % Set options to suppress quadprog output
    options = optimoptions('quadprog', 'Display', 'off');
    [z, ~, ~] = quadprog(F, f, Aineq, bineq, Aeq, beq, [], [], z0, options);
